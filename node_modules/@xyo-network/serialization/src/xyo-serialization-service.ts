...
import { BufferOrString, IOnTheFlyGetDataOptions, IParseResult, IXyoDeserializer, IXyoObjectSchema, IXyoSerializableObject, IXyoSerializationService } from "./@types"
...
export class XyoSerializationService extends XyoBase implements IXyoSerializationService {
  ...
  private recipes: Record<string, IXyoDeserializer<IXyoSerializableObject>> = {}
  ...
  public addDeserializer(deserializer: IXyoDeserializer<IXyoSerializableObject>) {
    if (this.recipes[ deserializer.schemaObjectId ]) {
  ...
  public arrayOf<T extends IXyoSerializableObject>(tCollection: T[]): IXyoSerializableObject {
    const typeAccumulator: Record<string, number> = {}

    for (const i of tCollection) {
      typeAccumulator[ i.schemaObjectId ] = (typeAccumulator[ i.schemaObjectId ] || 0) + 1
