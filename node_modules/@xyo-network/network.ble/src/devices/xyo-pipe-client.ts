...
export class XyoPipeClient implements IXyoNetworkPipe {
  ...
  public async tryCreatePipe(): Promise<IXyoNetworkPipe | void> {
    const timeout = new Promise<IXyoNetworkPipe | void>((_, reject) => {
    ...
    const promise = new Promise<IXyoNetworkPipe | void>(async (resolve, reject) => {
      try {
  ...
  public async read(characteristic: IXyoCharacteristic): Promise<Buffer> {
    const timeout = new Promise((_, reject) => {
      XyoBase.timeout(async () => {
        reject(`XyoPipeClient.read(): timeout() ${ characteristic.name }`)
      }, 12_000)
    }) as Promise<Buffer>
