...
export class XyoPipeClient implements IXyoNetworkPipe {
  ...
  public onPeerDisconnect(_callback: (hasError: boolean) => void): () => void {
    return () => {
      this.logger.info(`XyoPipeClient.onPeerDisconnect()`)
      // console.log(callback)
    }
  }

  public tryCreatePipe(): Promise<IXyoNetworkPipe> {
    const timeout = new Promise<IXyoNetworkPipe>((_, reject) => {
      XyoBase.timeout(() => {
        try {
          this.device.disconnect()
        } catch (e) {
          reject(`XyoPipeClient.tryCreatePipe(): timeout() ${ e }`) // Promise<null>; reject()
        }
      }, 10_000)
    })
    ...
    const promise = new Promise<IXyoNetworkPipe>(async (resolve, reject) => {
      try {
        ...
        if (Object.values(services).length === 1) {
          const characteristics = Object.values(services)[ 0 ].discoverCharacteristics()
  ...
  public send(data: Buffer, awaitResponse?: boolean): Promise<Buffer | undefined> {
    if (this.sessionCharacteristic != null) {
      this.chunkSend(data, this.sessionCharacteristic)
    ...
    const timeout = new Promise((_, reject) => {
      XyoBase.timeout(() => {
        try {
          ...
          reject(`XyoPipeClient.read(): timeout() ${ characteristic.uuid } ${ e }`)
        }
      }, 12_000)
    }) as Promise<Buffer>
  ...
  public chunkSend(data: Buffer, characteristic: IXyoCharacteristic): Promise<void> {
    ...
    const action = new Promise<void>((resolve) => {
      ...
      chunksToSend.forEach((_, index) => {
        characteristic.write(chunksToSend[ index ])
      })
