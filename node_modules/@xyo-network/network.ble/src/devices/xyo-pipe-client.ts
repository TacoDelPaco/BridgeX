...
export class XyoPipeClient implements IXyoNetworkPipe {
  ...
  public async tryCreatePipe(): Promise<IXyoNetworkPipe | null> {
    ...
    const timeout = new Promise<IXyoNetworkPipe | null>(async (_, reject) => {
      XyoBase.timeout(async () => {
        await this.device.disconnect() // await?
    ...
    const promise = new Promise<IXyoNetworkPipe | null>(async (resolve, reject) => {
      try {
        if (this.device.state !== 'connected') {
          await this.device.connect() // await
  ...
  public async send(data: Buffer, awaitResponse?: boolean): Promise<Buffer | undefined> {
    ...
    if (this.sessionCharacteristic != null) {
      ...
      if (awaitResponse !== false) {
        return await this.read(this.sessionCharacteristic)
      }
  ...
  public async read(characteristic: IXyoCharacteristic): Promise<Buffer> {
    // eslint-disable-next-line no-async-promise-executor
    const timeout = new Promise(async (_, reject) => {
      XyoBase.timeout(async () => {
    ...
    return await Promise.race([ timeout, action ])
  }
  ...
  public async chunkSend(data: Buffer, characteristic: IXyoCharacteristic): Promise<void> {
    ...
    // eslint-disable-next-line no-async-promise-executor
    const action = new Promise<void>(async (resolve) => {
      ...
      for (const [ , value ] of chunksToSend) {
        await characteristic.write(chunksToSend[ value ])
      }
