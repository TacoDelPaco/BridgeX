...
export class XyoPipeClient implements IXyoNetworkPipe {
  ...
  public async tryCreatePipe(): Promise<IXyoNetworkPipe | void | null> {
    const timeout = new Promise<IXyoNetworkPipe | void | null>((_, reject) => {
      XyoBase.timeout(() => {
        this.device.disconnect()
        reject() // Promise<null>; reject()
      }, 10_000)
    })

    // eslint-disable-next-line no-async-promise-executor
    const promise = new Promise<IXyoNetworkPipe | void | null>(async (resolve, reject) => {
      try {
  ...
  public read(characteristic: IXyoCharacteristic): Promise<Buffer | undefined> {
    const timeout = new Promise((_resolve, reject) => {
      XyoBase.timeout(() => {
        reject("XyoPipeClient.read(): timeout()")
      }, 12_000)
    }) as Promise<Buffer>
  ...
  public async chunkSend(data: Buffer, characteristic: IXyoCharacteristic): Promise<void> {
    ...
    const action = new Promise<void>((resolve) => {
      ...
      chunksToSend.forEach((_value, index) => {
