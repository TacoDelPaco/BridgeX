...
export class XyoClientBluetoothNetwork implements IXyoNetworkProvider {
    ...
    if (!this.tryingDevice && this.nearbyDevices.length > 0) {
      ...
      randomDevice.tryCreatePipe().then(async (createdPipe) => {
        ...
        if (createdPipe) {
          ...
          if (callback) {
            this.logger.info(`XyoClientBluetoothNetwork.callback`)
            callback()
            return
          }
        ...
        const callback = this.onClose
        if (callback) {
          this.logger.info(`XyoClientBluetoothNetwork.callback: Error`)
          callback()
          return
        }
  ...
  private async resolveCallback(pipe: IXyoNetworkPipe) {
  ...
  private async waitForJobToFinish() {
    ...
    return new Promise<void>(async (resolve) => {
      this.onClose = async () => {
        this.logger.info(`XyoClientBluetoothNetwork.onClose`)
        resolve(undefined)
      }
    })
  }
  ...
  public async findWithTimeout(timeoutInMills: number): Promise<IXyoNetworkPipe | void> {
    ...
    return new Promise(async (resolve, reject) => {
      this.logger.info(`XyoClientBluetoothNetwork.findWithTimeout(): return new Promise`)
      // default no async
      await this.scanner.startScan().then(async () => {
  ...
  public async find(_catalogue: IXyoNetworkProcedureCatalogue): Promise<IXyoNetworkPipe> {
    ...
    return new Promise(async (resolve) => {
      ...
      this.onPipe = async (pipe: IXyoNetworkPipe) => {
        this.logger.info(`XyoClientBluetoothNetwork.onPipe`)
        await this.shutDown() // default no async/await
        resolve(pipe)
      }
