...
import { IXyoBluetoothPeripheral, IXyoBluetoothPeripheralListener, IXyoMutableCharacteristic, IXyoMutableCharacteristicListener } from '@xyo-network/ble-peripheral'
import { XyoLogger } from '@xyo-network/logger'
import { IXyoNetworkPeer, IXyoNetworkPipe, IXyoNetworkProcedureCatalogue, IXyoNetworkProvider } from '@xyo-network/network'
...
export class XyoServerNetwork implements IXyoNetworkProvider {
    private onResume: (() => void) | undefined = undefined
    ...
    private deviceRouter: Record<string, XyoCharacteristicHandle> = {};
    private pipeCharacteristic: IXyoMutableCharacteristic
    private onNewPipe: ((pipe: IXyoNetworkPipe) => void) | undefined = undefined
    ...
    private closeHandler = async (id: string) => {
    ...
    public async findWithTimeout(timeoutInMills: number): Promise<IXyoNetworkPipe | void> {  
        ...
        return new Promise(async (resolve) => {
            ...
            this.server.startAdvertising(this.advData.advertisementData(), this.advData.getScanResponse()).then(async () => {
                ...
                this.logger.info(`XyoServerNetwork.findWithTimeout(): Error`)
                resolve(undefined)
            })
        })
    }
    ...
    public async find(_catalogue: IXyoNetworkProcedureCatalogue): Promise<IXyoNetworkPipe> { 
        this.logger.info("Find start for server")
