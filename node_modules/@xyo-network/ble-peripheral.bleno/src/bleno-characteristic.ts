import { Characteristic, CharacteristicOptions, Descriptor, Property } from '@abandonware/bleno'
...
export class BlenoCharacteristic implements IXyoMutableCharacteristic {
    ...
    onSubscribe = async (_maxValueSize: number, updateValueCallback: (value: Buffer) => void) => {
        this.logger.info("onSubscribe")
        this.notifyChangedCallback = updateValueCallback

        for await (const [ , value ] of this.listenersMap) {
    ...
    onReadRequest = async (_offset: number, callback: (result: number, data?: Buffer) => void) => {
    ...
    public async addListener(key: string, listener: IXyoMutableCharacteristicListener) {
        this.listenersMap.set(key, listener)
    }

    public async removeListener(key: string) {
