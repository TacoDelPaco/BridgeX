...
import { IXyoIterableStorageProvider, IXyoStorageProvider } from '@xyo-network/storage'
...
export class XyoOriginBlockRepository implements IXyoOriginBlockRepository {
  ...
  public async getOriginBlocks(limit: number, offsetHash?: Buffer): Promise<IOriginBlockQueryResult> {
    ...
    const promises: Promise<IXyoBoundWitness>[] = []
    let index = 0
  ...
  public async getBlocksThatProviderAttribution(hash: Buffer): Promise<Record<string, IXyoBoundWitness>> {
    ...
    return dest.reduce(async (promiseChain: Promise<Record<string, IXyoBoundWitness>>, result) => {
