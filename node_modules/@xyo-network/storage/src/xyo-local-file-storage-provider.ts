...
export class XyoLocalFileStorageProvider extends XyoBase implements IXyoStorageProvider {
  ...
  public async write(
    ...
    persist(this.dataFile, this.delegate.data)
    return
  }
...
function getOrCreateDb(dataFile: string): Record<string, Buffer> {
  const exists = existsSync(dataFile)
  if (exists) {
    const fileContents = readFileSync(dataFile, 'utf8')
    const json = JSON.parse(fileContents)

    return Object.keys(json).reduce((memo: Record<string, Buffer>, k) => {
...
function persist(dataFile: string, data: Record<string, Buffer>) {
  const transform = Object.keys(data).reduce((memo: Record<string, string>, key) => {
