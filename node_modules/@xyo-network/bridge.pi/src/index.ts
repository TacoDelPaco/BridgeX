...
import { XyoServerNetwork, XyoClientBluetoothNetwork, XyoFullBleNetwork, XyoAdvertisement } from '@xyo-network/network.ble'
...
import { XyoClientTcpNetwork, IXyoTCPNetworkAddress } from '@xyo-network/network.tcp'
...
const startBridge = async (service: InstanceType<typeof PrimaryService>) => {
  ...
  /* Checking for WiFi doesn't seem to work properly, testing if not true instead of true */
  while (!notOnWifi) {
    const status = await piWifi.getStatus()
