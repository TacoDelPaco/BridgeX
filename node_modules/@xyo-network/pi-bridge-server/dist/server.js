...
class BridgeServer {
    constructor(context, initializeRoutes = express_2.applyRoutes) {
        ...
        this.initContext = (req) => __awaiter(this, void 0, void 0, function* () {
            const token = lodash_1.get(req, 'req.headers.X-Auth-Token') || lodash_1.get(req, 'req.headers.x-auth-token');
            let authenticated = false;
            let authError = '';
            try {
                const pin = token && token_1.verify(token);
                authenticated = pin ? yield this.context.configuration.verifyPin(pin) : false;
            }
            catch (e) {
                authError = e.message;
            }
            return Object.assign({}, this.context, { authenticated,
                authError });
        });
