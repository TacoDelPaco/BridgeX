...
export abstract class XyoBase {
  ...
  public static stringify(value: unknown): string {
    return safeStringify(value, (_, v) => {
      if (v === '[Circular]') {
        return
      }
  ...
  public static timeout(fn: (...args: unknown[]) => void, timeMs: number, description?: string) {
    return this.schedule(setTimeout, clearTimeout, fn, timeMs, this.timeoutIds, description)
  }

  public static interval(fn: (...args: unknown[]) => void, timeMs: number, description?: string) {
    return this.schedule(setInterval, clearInterval, fn, timeMs, this.intervalIds, description)
  }

  public static immediate(fn: (...args: unknown[]) => void, description?: string) {
    return this.schedule(setImmediate, clearImmediate, fn, 0, this.immediateIds, description)
  }

  private static timeoutIds: Record<string, NodeJS.Timeout> = {}
  private static intervalIds: Record<string, NodeJS.Timeout> = {}
  private static immediateIds: Record<string, NodeJS.Immediate> = {}

  private static schedule<T>(
    scheduler: (callback: (...args: unknown[]) => void, ms: number, ...args: unknown[]) => T,
    unscheduler: (t: T) => void,
    fn: (...args: unknown[]) => void,
    timeMs: number,
    aggregator: Record<string, T>,
    description?: string
  ) {
  ...
  protected logInfo(message?: unknown, ...optionalParams: unknown[]) {
  ...
  protected logError(message?: unknown, ...optionalParams: unknown[]) {
    const resolvedOptionalParams = (optionalParams && optionalParams.length && optionalParams) || undefined
    if (resolvedOptionalParams) {
  ...
  protected logWarn(message?: unknown, ...optionalParams: unknown[]) {
    const resolvedOptionalParams = (optionalParams && optionalParams.length && optionalParams) || undefined
    if (resolvedOptionalParams) {
