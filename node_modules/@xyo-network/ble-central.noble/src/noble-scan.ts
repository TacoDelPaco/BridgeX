...
export class NobleScan implements IXyoScan {
  ...
  public async startScan(): Promise<void> {
    ...
    return new Promise(async (resolve, reject) => {
      if (noble.state === "poweredOn") {
        ...
        try { await noble.startScanningAsync([], true) }
  ...
  public async stopScan(): Promise<void> {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try { await noble.stopScanningAsync() }
  ...
  private stateChange = async (state: string) => {
    this.logger.info(`Noble state change: ${ state }`)
  }

  private async discover(peripheral: noble.Peripheral) {
