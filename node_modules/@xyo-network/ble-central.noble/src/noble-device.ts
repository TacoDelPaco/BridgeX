...
export class NobleDevice implements IXyoBluetoothDevice {
  ...
  public disconnect(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const callback = () => {
        this.logger.info(`disconnectAsync(): disconnect removeListener success`)
      }

      try {
        this.device.disconnectAsync()
        this.logger.info(`Disconnected from device with id: ${ this.device.id }`)
        this.device.removeListener(`disconnect`, callback)
        resolve()
      } catch {
        reject(`disconnectAsync(): Error`)
      }
    })
  }
  ...
  public discoverServicesForUuids(serviceUUIDs: string[]): Promise<IXyoService[]> {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        this.device.discoverServices(serviceUUIDs, (_error, services) => {
          ...
          this.device.on(`servicesDiscover`, callback)
