...
import { FetterOrWitness, IXyoBoundWitness, IXyoBoundWitnessParty, IXyoFetter, IXyoKeySet, IXyoSignatureSet, IXyoWitness } from "./@types"
...
export class XyoBoundWitness extends XyoBaseSerializable implements IXyoBoundWitness {
  ...
  constructor (public readonly fetterWitnesses: FetterOrWitness[],
    public readonly respectSign?: Buffer) {

    super(schema)
  }

  public stripMetaData(): IXyoBoundWitness {
    return new XyoBoundWitness(this.fetterWitnesses.map((fw): FetterOrWitness => {
    ...
    return {
      parties: this.publicKeys.map((publicKeySet, partyIndex) => {
        return {
          ...
          heuristics: this.heuristics[ partyIndex ].reduce((memo: Record<string, unknown>, heuristic) => {
            memo[ heuristic.getReadableName() ] = heuristic.getReadableValue()
            return memo
          }, {}),
          metadata: this.metadata[ partyIndex ].reduce((memo: Record<string, unknown>, metadata) => {
